...
class Pancreas(BaseDataset):
    ...
    BASE_DIR, NUM_CLASS, CROP_SIZE, PRESIZE = ('Task07_Pancreas/', 2, 256, True)
    IN_CHANNELS = 1
    CLASS_WEIGHTS = None
    mean = [0.07691266]
    std = [0.18697876]
            ...
            for root, dirs, files in os.walk(image_path):
                ...
                for file in files:
                    if '.nii.gz' in file:
                        continue
                    # image
                    # '../data/imgseg/Task07_Pancreas/imagesTr/pancreas_001/0.png'
                    image_dir = os.path.join(image_path, path[-1], file)
                    # mask
                    # '../data/imgseg/Task07_Pancreas/labelsTr/pancreas_001/0.png'
